...
export const api = {
  ...
  generateQuestionsStream: async (company, department, resume, { onStep, onDelta, onQuestion, onResult, onError }) => {
    ...
    let res
    try {
      res = await doFetch(directUrl, opts, true)
    } catch (_) {
      try {
        res = await doFetch(proxyUrl, opts, true)
      } catch (e) {
  ...
  // STT fallback - 浏览器环境走 Spring Boot /api/stt/transcribe（本期仅 Electron 桌面端可用，浏览器会抛错）
  transcribeAudio: async () => {
    throw new Error('STT 仅在桌面端 (Electron) 可用，请在设置页选择语音识别引擎并配置 API Key')
  },
