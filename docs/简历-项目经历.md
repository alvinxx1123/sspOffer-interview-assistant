...
**技术栈**：Java 17、Spring Boot 3.2、Spring Data JPA、Spring WebFlux、LangChain4j、H2、Docker、Nginx、Electron 桌面端  

**项目描述**：  
基于 RAG 与大模型的面经整合与 AI 面试准备 Web 应用，支持面经检索、AI 面试模拟、面试复盘、在线算法 IDE、简历与投递管理，并通过 Function Calling 让大模型可自动调用「查面经、查题库、跑代码」等后端能力；前后端分离，单 jar 部署，已上线。
...
- **部署与排错**：用 jlink 定制 JRE 17 + Electron 把 Spring Boot jar 打成一键安装包，跨机器分发不用装 JDK；公网 Piston 超时导致运行失败，改为 Docker 自建 Piston，通过环境变量注入 `PISTON_API_URL` 解决「Host is not specified」「Failed to fetch」等问题，并编写部署与排错文档
- **其他后端模块**：管理员密码鉴权（Filter 校验 `X-Admin-Password` 请求头）、面经/简历/投递的 CRUD 与 JPA 持久化、多模态接口解析面经图片/PDF 用于复盘

**项目亮点**：  
- 从需求分析、后端设计、前端对接到部署均由本人独立完成，具备从开发到线上排错的完整闭环实践  
...
**sspOffer 面经助手** | Java 17、Spring Boot 3.2、JPA、WebFlux、LangChain4j（Function Calling）、Docker、Electron 桌面端  
...
- **部署与排错**：jlink 定制 JRE + Electron 桌面端一键安装包，跨机器分发不用装 JDK；公网 Piston 超时，改为 **Docker 自建 Piston**，配合环境变量配置解决运行失败；实现管理员鉴权（Filter）、面经/简历/投递 CRUD 与 JPA 持久化、多模态解析用于复盘；项目已上线
...
**回答要点**：  
项目是面向互联网求职的面试准备平台，核心是面经检索 + AI 出题 + 在线写算法。我负责后端整体设计与实现：RAG 面经检索与 AI 面试模拟、在线 IDE 依赖的代码执行服务、管理员鉴权、面经/简历/投递的 CRUD，以及部署到和线上代码执行超时后的 Docker 自建 Piston 方案与文档。

**2. 为什么做这个项目？技术选型怎么考虑的？**

**回答要点**：  
自己找实习时需要整理面经和练算法，希望有一个能按公司/部门查面经、并能根据简历和面经自动出题的练习环境。技术选型上：后端用 Spring Boot 3 + JPA 快速落地 CRUD 和持久化；RAG 用 LangChain4j 做向量检索和 LLM 调用，减少自己写 prompt 与 embedding 的细节；代码执行用开源 Piston 避免自建沙箱；分发用 jlink 定制 JRE + Electron 桌面端一键安装包，跨机器不用装 JDK。
...
解决方式是：本地用 Docker 自建 Piston，把 PISTON_API_URL 配成 http://127.0.0.1:2000/api/v2，这样后端只访问本机，延迟低；若用 Nginx 反向代理则把 proxy_read_timeout 设大一点（如 60s 以上），避免代理先超时。并写了文档记录部署步骤和这些报错的排查方法。
